...
연결된 모든 소스(Drive·Slack·메일 등) 횡단 검색 — 질의 분해·다중 소스 검색·중복 제거 종합·출처 명기·활동 다이제스트까지 한 스킬로 제공합니다. 한국 표준 + UZ 듀얼 컨텍스트를 적용합니다.

**한국화 노트**: 원본 5스킬(search·search-strategy·knowledge-synthesis·source-management·digest) 통합. 커넥터 미연결 시 연결 안내 후 로컬 폴더 검색으로 폴백.

**도구 일반화**: 원문의 미국 SaaS 연동은 방법론으로만 계승한다 — 한국 실무에서는 연결된 커넥터·업로드 문서·사용자 제공 정보를 소스로 쓴다.

**UZ 컨텍스트**: UZ: 다국어 혼용 문서(KO/RU/UZ) 검색 시 언어별 쿼리 변형 생성 규칙. 상세는 `references/uz-enterprise-search.md`.

---

## 원문 방법론 (EN, knowledge-work-plugins)

<!-- source: enterprise-search/search -->
# Search Command
...
---
<!-- source: enterprise-search/search-strategy -->
# Search Strategy
...
---
<!-- source: enterprise-search/knowledge-synthesis -->
# Knowledge Synthesis
...
---
<!-- source: enterprise-search/source-management -->
# Source Management
...
---
<!-- source: enterprise-search/digest -->
# Digest Command
