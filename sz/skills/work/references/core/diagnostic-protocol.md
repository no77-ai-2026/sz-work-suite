...
**플러그인 인벤토리 검사**: 설치된 SZ 플러그인(`gil`·`gil-creative`·`gil-commerce`)의 plugin.json·skills/를 실측 대조한다. 스냅샷(.sz/config.json)과 어긋나면 경고한다.
...
**부분 재설정**: `AGENTS.md`만 재생성(스킬 체인 재설계) — `/work` 재실행. 커넥터 키만 재등록 — "API 키 설정할래" 자연어 안내.

**전체 재설정**: `/work` 재실행(Phase 1 인터뷰 재실행, S3 재진입 확인 후 진행).
